...
public class UserEmpruntAjout {
    ...
    public void initialize(String adherentId) {
        ...
        champ2Field.setEditable(false); // Rendre le champ non modifiable
        addComponent(panel, champ2Label, constraints, 0, 2, 1, 1);
        addComponent(panel, champ2Field, constraints, 1, 2, 1, 1);

        JLabel champ3Label = new JLabel("Statut de l'emprunt:");
        champ3Field = new JTextField(20);
        champ3Field.setEditable(false); // Rendre le champ non modifiable
        champ3Field.setText("emprunter"); // Définir la valeur par défaut
        addComponent(panel, champ3Label, constraints, 0, 3, 1, 1);
        addComponent(panel, champ3Field, constraints, 1, 3, 1, 1);

        // Champ pour le numéro d'adhérent
        JLabel champ4Label = new JLabel("Numéro de l'adhérent:");
        JTextField adhNumField = new JTextField(adherentId);
        adhNumField.setEditable(false); // Empêcher l'édition par l'utilisateur
        addComponent(panel, champ4Label, constraints, 0, 4, 1, 1);
        addComponent(panel, adhNumField, constraints, 1, 4, 1, 1);
        ...
        validerButton.addActionListener(e -> validerAjoutEmprunt());
    ...
    private void validerAjoutEmprunt() {
        // Récupérer les valeurs des champs
        String dateEmprunt = champ1Field.getText();
        String dateRetour = champ2Field.getText();
        String statutEmprunt = champ3Field.getText(); // Récupérer le statut directement depuis le champ
        ...
        if (!statutEmprunt.equalsIgnoreCase("rendu") && !statutEmprunt.equalsIgnoreCase("emprunter")) {
        ...
        boolean statutEmpruntBool = statutEmprunt.equalsIgnoreCase("rendu");
    ...
    public static void main(String[] args) {
        UserEmpruntAjout empruntAjout = new UserEmpruntAjout();
        // Vous devez fournir l'identifiant de l'adhérent ici, ou bien récupérer
        // l'identifiant à partir d'une autre source
        String adherentId = "votre_id_adherent";
        empruntAjout.initialize(adherentId);
    }
}
