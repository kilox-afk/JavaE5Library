...
public class AdminEmpruntModificationInterface {
    ...
    public AdminEmpruntModificationInterface(int empruntId) {
    ...
    public void initialize() {
        ...
        JLabel dateEmpruntLabel = new JLabel("Date de l'emprunt:");
    ...
    private void chargerAdhNum() {
        List<String> adhNumList = new ArrayList<>();
        try (Connection conn = DriverManager.getConnection(URL, USERNAME, PASSWORD)) {
            String sql = "SELECT Adh_num FROM adherent";
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                try (ResultSet rs = stmt.executeQuery()) {
                    while (rs.next()) {
    ...
    private void modifierEmprunt() {
        ...
        if (!isValidDateFormat(nouveauDateEmprunt) || (nouveauDateRetour != null && !isValidDateFormat(nouveauDateRetour))) {
    ...
    public static void main(String[] args) {
        ...
        AdminEmpruntModificationInterface empruntModificationInterface = new AdminEmpruntModificationInterface(empruntId);
